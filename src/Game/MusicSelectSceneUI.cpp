...
namespace FlappyBird
{
		...
		: IComponent(owner), m_selectedMusicIndex(0)
	{
		...
		CreateMusicChangeButtons();

		// 曲変更時の効果音
		m_changeMusicSound = m_owner.lock()->AddComponent<SoundClip>(m_owner.lock());
		m_changeMusicSound->LoadWavSound(L"res/sound/music_change.wav");
	}
	...
	void MusicSelectSceneUI::CreateMusicChangeButtons()
	{
		auto& viewport = Dx12GraphicsEngine::GetViewport();
